...
import {
  ...
  Eye, Sparkles, FileSpreadsheet, Copy, Check, Search, MessageSquare,
  Smartphone, CheckCheck
} from "lucide-react";
import communicationAPI, {
  type EmailSettings, type DnsRecord, type EmailTemplateSummary,
  type BulkRecipient, type ScheduledDispatchItem, type WhatsAppConfig, type WhatsAppTemplateItem
...
type Tab = "manual" | "bulk" | "auto" | "scheduled" | "templates" | "config" | "wsp_send" | "wsp_bulk" | "wsp_templates" | "wsp_config";
...
export default function EmailPage() {
  ...
  const emailTabs = [
    ...
    { key: "templates" as Tab, title: "Email Templates", icon: FileText },
    { key: "config" as Tab, title: "Domain & DNS", icon: ShieldCheck },
  ];

  const wspTabs = [
    { key: "wsp_send" as Tab, title: "WhatsApp Direct", icon: MessageSquare, badge: "WSP" },
    { key: "wsp_bulk" as Tab, title: "WhatsApp Bulk", icon: Users, badge: "Mass" },
    { key: "wsp_templates" as Tab, title: "Meta Templates", icon: FileText },
    { key: "wsp_config" as Tab, title: "Meta Account Config", icon: Smartphone },
  ];

  const handleChannelSwitch = (ch: Channel) => {
    setChannel(ch);
    if (ch === "email") setTab("manual");
    else setTab("wsp_send");
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">
            Manage Email replies & WhatsApp Business Cloud API messages (+91 91766 00994) across your organization.
          </p>
        </div>

        {/* CHANNEL SWITCHER PILL */}
        <div className="flex items-center gap-1 p-1 rounded-2xl bg-slate-200/80 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 shrink-0">
          <button
            type="button"
          ...
          >
            <MessageSquare size={14} /> WhatsApp (WSP)
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 p-1.5 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-inner w-full overflow-x-auto scrollbar-none">
        {(channel === "email" ? emailTabs : wspTabs).map((t) => {
          const Icon = t.icon;
...
function WhatsAppDirectSendSection() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(`WhatsApp message dispatched successfully via Meta Cloud API (+91 91766 00994)! ID: ${res.whatsapp_msg_id || 'wmid'}`);
      setToPhone("");
      setToName("");
      setMessage("");
    } catch (e) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {/* WHATSAPP INTERACTIVE PHONE CARD PREVIEW */}
      <div>
        <section className={card}>
          <div className={cardHead}>
            <div className="flex items-center gap-2">
